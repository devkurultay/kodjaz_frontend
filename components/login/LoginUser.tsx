...
export default function LoginUser() {
  ...
  const { shouldConfirmationPopup } = useAppSelector(userState);

  function checkIfClickedOutside(e: Event) {
  ...
  return (
    <div
      className={`flex min-h-full items-center justify-center py-12 px-4 sm:px-6 lg:px-8 fixed top-0 left-0 w-full z-50 bg-blackColor/50 overflow-hidden ${
        shouldConfirmationPopup ? 'block' : 'hidden'
      }`}
    >
      <div className={`w-full max-w-[400px] bg-whiteColor p-[30px] rounded-[20px] ${styles.popup}`} ref={ref}>
        <div className="flex justify-end">
          <button
            className="mr-[-15px] mt-[-12px]"
            onClick={() => {
              dispatch(closeConfirmationPopup());
            }}
          >
        ...
        <form onSubmit={handleSubmit(submitHandler)}>
          ...
          <div className="flex gap-x-4 justify-center mb-[30px]">
            ...
            <button className="border rounded-md border-grayColorDb flex justify-center items-center w-[50px] h-[50px]">
              <FacebookIcon fill="#3872DC" width={17} height={37} className="mt-2 ml-1" />
            </button>
          </div>
